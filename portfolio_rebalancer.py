...
class PortfolioRebalancer:
    ...
    def log_rebalancing_transactions(
        self,
        actions: List[RebalancingAction],
        ...
        """
        Log rebalancing actions as transactions in the transaction tracker
        
        Args:
            actions: List of rebalancing actions to log
            transaction_tracker: TransactionTracker instance to log trades
            fee_percentage: Trading fee as percentage (e.g., 0.1 for 0.1%)
            exchange: Exchange/platform name for transactions
            
        Returns:
            List of transaction IDs that were created
        """
        if not TRANSACTION_TRACKER_AVAILABLE:
            print("Warning: Transaction tracker not available. Transactions will not be logged.")
            return []
        
        if transaction_tracker is None:
            print("Warning: No transaction tracker provided. Transactions will not be logged.")
            return []
        
        transaction_ids = []
        
        for action in actions:
            if action.action == "BUY" and action.value_diff > 0 and action.amount_diff > 0:
                # Calculate fee
                fee = action.value_diff * (fee_percentage / 100.0) if fee_percentage > 0 else 0.0
                
                trans_id = transaction_tracker.record_transaction(
                    symbol=action.symbol,
                    transaction_type=TransactionType.BUY,
                    amount=action.amount_diff,
                    price_per_unit=action.current_price,
                    fee=fee,
                    exchange=exchange,
                    notes=f"Rebalancing: Buy to reach {action.target_allocation}% allocation (current: {action.current_allocation:.2f}%)"
                )
                transaction_ids.append(trans_id)
                
            elif action.action == "SELL" and action.value_diff < 0 and action.amount_diff < 0:
                # Calculate fee
                sell_value = abs(action.value_diff)
                fee = sell_value * (fee_percentage / 100.0) if fee_percentage > 0 else 0.0
                
                trans_id = transaction_tracker.record_transaction(
                    symbol=action.symbol,
                    transaction_type=TransactionType.SELL,
                    amount=abs(action.amount_diff),
                    price_per_unit=action.current_price,
                    fee=fee,
                    exchange=exchange,
                    notes=f"Rebalancing: Sell to reach {action.target_allocation}% allocation (current: {action.current_allocation:.2f}%)"
                )
                transaction_ids.append(trans_id)
