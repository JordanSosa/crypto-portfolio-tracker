...
class TransactionTracker:
    ...
    def get_portfolio_pnl_summary(
        ...
        # Check if price fetching failed (no unrealized P&L data but we should have positions)
        prices_failed = False
        if not unrealized_pnl:
            # Check if we actually have open positions
            cursor = self.conn.cursor()
            if symbols is None:
                cursor.execute("""
                    SELECT DISTINCT symbol
            ...
            if symbols:  # We have positions but no P&L data = prices failed
                prices_failed = True
                # Get cost basis data as fallback
                cost_basis_data = self.get_portfolio_cost_basis()
                total_cost_basis = sum(cb['total_cost_basis'] for cb in cost_basis_data.values())
                ...
                return {
                    ...
                    'total_gain_loss': realized_pnl['total_realized_pnl'],
                    'total_cost_basis': total_cost_basis,
                    'total_current_value': 0,  # Can't calculate without prices
                    'total_return_pct': 0,
                    'prices_failed': True
                }
        
        # Calculate totals (normal case with prices)
